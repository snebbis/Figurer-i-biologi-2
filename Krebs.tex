...
\draw[line width=3.2pt, arrowGray, line cap=round, ->]
    (krebs.center) ++(75:1.4cm) arc[start angle=75, end angle=422, radius=1.4cm];
...
\draw[arrow] (acetyl.south)  -- ([yshift=1.4cm]krebs.center);
...
% Glyserol -> Glycolysis (Reverted to the original orthogonal routing)
\draw[arrow] (glyserol.east) -- ++(0.5,0) |- ([yshift=0.15cm]pyruvat.west);
...
\draw[arrow] (\colT,\rowKrebs) -- (\colC-1.45,\rowKrebs);

% Branch 2 -> Glycolysis 
\draw[arrow] (\colT,\rowTrunkTop) -- ([yshift=-0.15cm]pyruvat.west);

% =====================================================
% Background colored regions (Strictly anchored to \colRespWest)
% =====================================================
\begin{scope}[on background layer]
    % Glykolyse Background
    \node[fill=glykbg, minimum width=\respWidth, minimum height=4.2cm, anchor=north west]
          (glykbox) at (\colRespWest, 1.4) {};
          
    % Krebs-syklus Background
    \node[fill=krebsbg, minimum width=\respWidth, minimum height=5.2cm, anchor=north west]
          (krebsbox) at (\colRespWest, -5.2) {};
\end{scope}
